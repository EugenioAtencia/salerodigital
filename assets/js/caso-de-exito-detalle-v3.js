(function () {
  ...
  async function loadCase() {
    ...
    try {
      const json = await fetchJson(ajaxUrl);
      return json && json.success ? json.data : json;
    } catch (ajaxError) {}

    const restUrl = `${CMS_ORIGIN}/wp-json/wp/v2/casos-exito?slug=${encodeURIComponent(slug)}&_embed=1&_t=${Date.now()}`;
  ...
  function render(item = {}) {
    ...
    const title = textField(source, ['cliente_nombre', 'nombre_caso', 'nombre_cliente', 'cliente'], valueFrom(item.title) || 'Caso de éxito');
    ...
    const reto = htmlField(source, ['reto', 'reto_inicial']);
    const solucion = htmlField(source, ['solucion', 'estrategia_aplicada', 'acciones_realizadas', 'receta_aplicada']);
    const resultado = htmlField(source, ['resultado', 'resultados']);
    const aprendizaje = htmlField(source, ['que_demuestra_este_caso', 'que_demuestra', 'aprendizaje', 'lectura_estrategica']);
