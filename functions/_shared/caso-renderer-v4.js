...
function buildWpUrls(endpoint, params = {}) {
  ...
  const pretty = new URL(`${CMS_API_BASE}/${normalizedEndpoint}`);
  Object.entries(params).forEach(([key, value]) => pretty.searchParams.set(key, String(value)));

  const restRoute = new URL(`${CMS_ORIGIN}/`);
  restRoute.searchParams.set('rest_route', `/wp/v2/${normalizedEndpoint}`);
  Object.entries(params).forEach(([key, value]) => restRoute.searchParams.set(key, String(value)));
...
async function fetchMediaById(id) {
  ...
  const media = await fetchWpJson(`media/${id}`, { _t: Date.now() });
...
function renderSection(title, html, modifier = '', number = '') {
  ...
  return `<section class="caso-detail-section${mod}"><span class="caso-section-number">${escapeHtml(number)}</span><h3>${escapeHtml(title)}</h3>${html}</section>`;
}
