...
export async function handleSectorRequest(context) {
  ...
  try {
    ...
    return htmlResponse(renderErrorPage('No se pudo cargar el contenido desde WordPress', 'La landing existe como ruta dinámica, pero WordPress no ha respondido a tiempo. Revisa el CMS o inténtalo de nuevo en unos minutos.'), 503);
  }
}

async function fetchSector(slug) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort('CMS timeout'), CMS_TIMEOUT_MS);
  ...
  try {
    ...
    const response = await fetch(url.toString(), {
      ...
      headers: {
        ...
        'User-Agent': 'SaleroDigital-Sectors-SSR'
      },
      cf: {
        cacheTtl: 0,
...
function renderSectorPage(slug, item) {
  ...
  const kind = sectorKind(slug, item);
  ...
  const metaTitle = withBrand(stripHtml(fieldValue(acf, ['meta_title', 'og_title'], title)));
...
<head>
  ...
  ${renderJsonLd({ slug, title, claim, metaDescription, canonical, faqs: parseFaqs(acf.faqs_repeater || acf.preguntas_frecuentes || acf.faqs_items || acf.faqs) })}
</head>
...
function renderHeader() {
  return `  <header class="site-header"><div class="container header-inner"><a class="logo logo-wordmark" href="/" aria-label="Salero Digital"><span>Salero Digital</span></a><nav class="nav" aria-label="Menú principal"><a href="/el-menu/">El Menú</a><a href="/nuestros-menus/">Nuestros menús</a><a href="/sectores/" class="is-active" aria-current="page">Sectores</a><a href="/la-receta/">La Receta</a><a href="/la-rebotica/">La Rebotica</a><a class="nav-mobile-contact" href="/hablamos/">¿Hablamos?</a><a class="nav-mobile-cta" href="/hablamos/">Pide tu cata digital</a></nav><div class="header-actions"><a class="nav-contact" href="/hablamos/">¿Hablamos?</a><a class="btn btn-primary" href="/hablamos/">Pide tu cata digital</a><button class="menu-toggle" type="button" data-menu-toggle aria-label="Abrir menú">☰</button></div></div></header>`;
}
...
function renderHeroMedia(videoUrl, posterUrl, title) {
  if (videoUrl) return `<video class="sector-detail-hero-video" data-sector-video autoplay muted loop playsinline preload="metadata" ${posterUrl ? `poster="${escapeAttr(posterUrl)}"` : ''} aria-hidden="true"><source src="${escapeAttr(videoUrl)}" type="video/mp4"></video>`;
  if (posterUrl) return `<img class="sector-detail-hero-image" src="${escapeAttr(posterUrl)}" alt="${escapeAttr(stripHtml(title))}" loading="eager">`;
  return '';
}
...
function renderStrategySection(acf) {
  ...
  return cards ? `<section class="sector-strategy-section"><div class="sector-block-heading"><span class="sector-section-kicker">Plan de acción</span><h2>De la presencia digital a la captación real</h2></div><div class="sector-strategy-grid">${cards}</div></section>` : '';
}

function renderStrategyCard(acf, key, title, number) {
  const items = fieldList(acf[key]);
  return items.length ? `<article class="sector-strategy-card sector-strategy-${escapeAttr(key)}"><span>${number}</span><h2>${escapeHtml(title)}</h2><ul>${items.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul></article>` : '';
}

function renderFaqBlock(acf) {
  const faqs = parseFaqs(acf.faqs_repeater || acf.preguntas_frecuentes || acf.faqs_items || acf.faqs);
...
function parseFaqs(value) {
  ...
  if (typeof value === 'object') {
    const q = value.pregunta || value.question || value.titulo || value.title || '';
    const a = value.respuesta || value.answer || value.texto || value.content || '';
    return q ? [{ q: stripHtml(String(q)).trim(), a: String(a || '').trim() }] : [];
  }
  return [];
}

function renderJsonLd({ title, metaDescription, canonical, faqs }) {
  const graph = [
    { '@type': 'WebPage', '@id': `${canonical}#webpage`, url: canonical, name: title, description: metaDescription, isPartOf: { '@type': 'WebSite', '@id': `${SITE_ORIGIN}/#website`, name: 'Salero Digital', url: SITE_ORIGIN } },
    { '@type': 'BreadcrumbList', '@id': `${canonical}#breadcrumb`, itemListElement: [ { '@type': 'ListItem', position: 1, name: 'Inicio', item: `${SITE_ORIGIN}/` }, { '@type': 'ListItem', position: 2, name: 'Sectores', item: `${SITE_ORIGIN}/sectores/` }, { '@type': 'ListItem', position: 3, name: title, item: canonical } ] }
  ];
  if (faqs.length) graph.push({ '@type': 'FAQPage', '@id': `${canonical}#faq`, mainEntity: faqs.map(faq => ({ '@type': 'Question', name: faq.q, acceptedAnswer: { '@type': 'Answer', text: stripHtml(faq.a) } })) });
...
function sectorFallback(kind = 'generico') {
  const base = { eyebrow: 'Sectores', cardTitle: 'Una estrategia con el punto justo para tu sector.', cardItems: ['Visibilidad local más clara', 'Contenido con intención comercial', 'Medición sencilla y útil'], ctaLabel: 'Pide tu cata digital', finalTitle: 'Tu negocio ya tiene oficio. Ahora toca que se vea como merece.', finalText: 'Revisamos tu presencia digital y te decimos qué acciones pueden ayudarte a ganar visibilidad, confianza y oportunidades reales.', sidebarItems: ['Cómo apareces en Google y Google Maps', 'Qué transmite tu web en los primeros segundos', 'Cómo comunicas en redes sociales', 'Qué hace tu competencia directa', 'Dónde se están perdiendo oportunidades'] };
  if (kind === 'hosteleria') return { ...base, eyebrow: 'Hostelería y turismo', heroTitle: 'Marketing para negocios que se viven en la mesa, en la reserva y en la experiencia', heroText: 'Estrategias digitales para restaurantes, bares, cafeterías, alojamientos rurales y proyectos turísticos que quieren ganar visibilidad, confianza y oportunidades reales sin perder su forma de ser.', cardTitle: 'Tu negocio ya tiene encanto. Ahora toca que se encuentre, se entienda y se elija.', fallbackVideo: '/assets/video/hosteleria-hero.mp4', fallbackPoster: '/assets/img/hosteleria-hero-poster.webp' };
  if (kind === 'comercio') return { ...base, eyebrow: 'Comercios y pymes', heroTitle: 'Marketing para comercios que necesitan más visibilidad local y más ventas', heroText: 'Estrategias digitales para tiendas, negocios de barrio y pymes que quieren aparecer mejor, comunicar con más claridad y convertir la cercanía en oportunidades reales.', cardTitle: 'Tu comercio ya tiene trato y producto. Ahora toca que te encuentren antes.' };
  if (kind === 'aceite') return { ...base, eyebrow: 'Almazaras y aceite', heroTitle: 'Marketing para marcas con origen, producto y mucho que contar', heroText: 'Estrategias digitales para almazaras, cooperativas y proyectos agroalimentarios que quieren poner en valor su producto, su territorio y su diferencia.', cardTitle: 'Tu producto tiene origen. Ahora toca convertirlo en marca y demanda.' };
  return base;
}
...
function excerpt(item = {}) { return item.excerpt && item.excerpt.rendered ? stripHtml(item.excerpt.rendered) : ''; }
function featuredImage(item = {}) { const media = item._embedded && item._embedded['wp:featuredmedia']; return item.featured_image_url || (media && media[0] && media[0].source_url ? media[0].source_url : ''); }
function fieldValue(obj, keys, fallback = '') { for (const key of keys) { const value = obj && obj[key]; if (value !== undefined && value !== null && value !== '') return value; } return fallback; }
function fieldUrl(value) { if (!value) return ''; if (typeof value === 'string') return value.trim(); if (Array.isArray(value)) return fieldUrl(value[0]); if (typeof value === 'object') return value.url || value.source_url || value.guid?.rendered || value.sizes?.full || value.sizes?.large || value.sizes?.medium_large || ''; return ''; }
function fieldList(value) { if (!value) return []; if (Array.isArray(value)) return value.map(item => typeof item === 'string' ? item : item.texto || item.titulo || item.title || item.label || item.nombre || '').filter(Boolean).map(stripHtml); if (typeof value === 'string') return value.split(/\r?\n|\|/).map(item => stripHtml(item.trim())).filter(Boolean); return []; }
function normalizeUrl(value = '') { const url = String(value || '').trim(); if (!url) return '/hablamos/'; if (url.startsWith('http') || url.startsWith('/') || url.startsWith('#')) return url; return `/${url.replace(/^\/+/, '')}`; }
function withBrand(title = '') { const clean = stripHtml(title || ''); return clean.toLowerCase().includes('salero') ? clean : `${clean} | Salero Digital`; }
function formatText(value = '') { const text = String(value || '').trim(); if (!text) return ''; if (/<[a-z][\s\S]*>/i.test(text)) return text; return text.split(/\n{2,}/).map(p => `<p>${escapeHtml(p.trim())}</p>`).join(''); }
