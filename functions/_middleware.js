import { renderJsonLd, schemaForPath, schemaGraph, organizationSchema, websiteSchema, webPageSchema, breadcrumbSchema, blogPostingSchema, SITE_ORIGIN } from './_shared/schema.js';
...
export async function onRequest(context) {
  ...
  const schema = schemaForPath(requestUrl.pathname) || schemaForBlogArticle(requestUrl.pathname, html);
...
function extractArticleText(html = '') {
  const content = String(html).match(/<div class=["']ba-content["'][^>]*>([\s\S]*?)<\/div>\s*(?:<section class=["']ba-inline-faq|<\/div>\s*<\/div>\s*<\/section>)/i);
  return stripHtml(content ? content[1] : html);
}
