...
const CASE_DETAIL_CARD_FIX_CSS = '<link rel="stylesheet" href="/assets/css/case-detail-card-fix.css?v=1">';
...
export async function onRequest(context) {
  ...
  nextHtml = injectCaseDetailAssets(nextHtml, normalizedPath);
...
function injectCaseDetailAssets(html = '', path = '') {
  if (!isCaseDetailPath(path)) return html;
  return injectOrReplaceStylesheet(html, '/assets/css/case-detail-card-fix.css', CASE_DETAIL_CARD_FIX_CSS);
}
...
function isCaseDetailPath(pathname = '/') {
  const normalized = normalizePath(pathname);
  return /^\/casos-de-exito\/[^/]+\/$/.test(normalized);
}
